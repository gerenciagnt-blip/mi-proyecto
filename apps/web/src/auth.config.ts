...
// Timeout de inactividad: la sesión JWT expira a los 15 minutos desde la
...
// expiración. Si el usuario se queda quieto > 15 min → el middleware
// detecta la sesión vencida y redirige a /login.
const SESSION_MAX_AGE_SECONDS = 15 * 60; // 15 minutos
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        token.asesorComercialId = user.asesorComercialId;
      }
      return token;
    },
    async session({ session, token }) {
      if (token && session.user) {
        ...
        session.user.asesorComercialId = token.asesorComercialId;
      }
      return session;
    },
  },
