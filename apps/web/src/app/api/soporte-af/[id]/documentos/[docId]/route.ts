...
/**
 * GET /api/soporte-af/[id]/documentos/[docId] — sirve un documento
 * adjunto de una solicitud Soporte · Afiliaciones. Scope:
 *   - SUCURSAL (aliado) sólo puede descargar docs de solicitudes de su
 *     sucursal.
 *   - STAFF puede descargar cualquiera.
 *   - Archivos eliminados por retención 120d responden 410 Gone.
...
export async function GET(
  ...
  const doc = await prisma.soporteAfDocumento.findUnique({
    where: { id: docId },
    select: {
      id: true,
      soporteAfId: true,
      ...
      eliminado: true,
      soporteAf: { select: { sucursalId: true } },
    },
  });
  if (!doc || doc.soporteAfId !== id) {
  ...
  // Scope: aliado sólo documentos de su sucursal.
  ...
  if (
    scope.tipo === 'SUCURSAL' &&
    doc.soporteAf.sucursalId !== scope.sucursalId
  ) {
    return NextResponse.json(
