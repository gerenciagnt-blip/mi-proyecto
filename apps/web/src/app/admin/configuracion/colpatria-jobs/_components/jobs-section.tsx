/**
 * Sección "Jobs" del módulo Bot Colpatria — listado paginado de jobs
 * con filtros (estado, empresa, documento) + tabla + acciones por fila.
 *
 * Server Component. La página padre (`../page.tsx`) le pasa los
 * searchParams ya parseados.
 */
...
export type JobsSectionParams = {
  status?: string;
  empresaId?: string;
  documento?: string;
  page?: string;
  /** Sección actual — se incluye en los URLs de filtro/paginación
   *  para conservar el contexto de tab al navegar. */
  seccion?: string;
};

export async function JobsSection({ sp }: { sp: JobsSectionParams }) {
  ...
  function urlCon(patch: Partial<JobsSectionParams>): string {
    ...
    if (final.seccion) qs.set('seccion', final.seccion);
