...
import { ArrowLeft, Landmark, AlertCircle, Upload } from 'lucide-react';
...
export default async function MovimientosPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  const [movimientos, statsByEstado, bancos] = await Promise.all([
          ...
          <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            {movimientos.length === 0 ? (
              <Alert variant="info" className="m-5">
                ...
                <span>Sin movimientos. Importa un extracto para comenzar.</span>
              </Alert>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead className="text-left text-xs uppercase tracking-wider text-slate-500">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-right">Det.</th>
